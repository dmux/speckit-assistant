...
export const HumanReviewModal: React.FC<HumanReviewModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      {/* Modal Card */}
      <div className="bg-zinc-950 border border-zinc-850 w-full h-[95vh] rounded-xl flex flex-col overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-200">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-blue-500/10 text-blue-400 border border-blue-500/20 uppercase animate-pulse">
                  Awaiting Sign-off
                </span>
              </div>
        ...
        {/* Error banner if any */}
        {error && (
          <div className="mx-6 mt-4 p-3 bg-red-500/10 border border-red-500/20 rounded-lg flex items-center gap-2 text-xs text-red-400">
        ...
        {/* Quick Stats Grid */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 px-6 py-4 border-b border-zinc-850 bg-zinc-950">
          <div className="p-3 bg-zinc-900/40 border border-zinc-850/50 rounded-lg flex items-center gap-3">
        ...
        <div className="flex-1 flex overflow-hidden min-h-0">
          ...
          <div className="w-80 border-r border-zinc-850 flex flex-col bg-zinc-950 shrink-0">
            {/* Tabs */}
            <div className="flex border-b border-zinc-850 bg-zinc-900/20">
              <button
                onClick={() => setActiveTab('files')}
                className={`flex-1 py-2.5 text-center text-xs font-semibold border-b-2 transition ${activeTab === 'files' ? 'text-white border-white' : 'text-zinc-500 border-transparent hover:text-zinc-350'}`}
              >
                Modified Files
              </button>
              <button
                onClick={() => setActiveTab('git')}
                className={`flex-1 py-2.5 text-center text-xs font-semibold border-b-2 transition ${activeTab === 'git' ? 'text-white border-white' : 'text-zinc-500 border-transparent hover:text-zinc-350'}`}
              >
                Git Commit History
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-1">
              {activeTab === 'files' ? (
                  ...
                  files.map((file) => (
                    ...
                    >
                      <div className="flex items-center gap-2 min-w-0">
                        <FileText size={13} className={file.type === 'code' ? 'text-blue-400' : 'text-zinc-500'} />
                ...
                <div className="space-y-4 py-2">
                  ...
                  <div className="p-3 bg-zinc-900/30 border border-zinc-850 rounded-lg flex items-center gap-2 mb-2">
                    <GitBranch size={14} className="text-blue-500 animate-pulse" />
                    <span className="text-xs text-zinc-300 font-bold">{branchName}</span>
                  ...
                  {isLoading ? (
                    ...
                    gitLog.map((commit, idx) => (
                      <div key={commit.hash} className="flex gap-3 relative pl-1">
                        {/* Timeline node line */}
                        {idx < gitLog.length - 1 && (
                          <div className="absolute left-2.5 top-5 bottom-0 w-px bg-zinc-800" />
                        )}
                        <div className="relative z-10 mt-1">
                          <GitCommit size={12} className="text-blue-500 bg-zinc-950 rounded-full" />
                        </div>
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[11px] font-semibold text-zinc-200 truncate mt-0.5 font-sans font-medium">
                            {commit.message}
                          </p>
          ...
          <div className="flex-1 flex flex-col bg-zinc-900/20 overflow-hidden">
            <div className="px-4 py-2.5 bg-zinc-950 border-b border-zinc-850 flex justify-between items-center">
              <span className="text-xs font-mono text-zinc-300 font-bold">
                {selectedFile || 'No file selected'}
              </span>
              <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-wide bg-zinc-900 border border-zinc-850 px-2 py-0.5 rounded">
                Unified Diff
              </span>
            </div>

            <div className="flex-1 p-4 overflow-auto font-mono text-[11px] leading-relaxed text-zinc-300 bg-black/60 select-text">
              {fileDiff ? (
                fileDiff.split('\n').map((line, idx) => {
                  let lineClass = 'text-zinc-400';
                  if (line.startsWith('+')) lineClass = 'text-green-500 bg-green-500/5 font-semibold';
                  else if (line.startsWith('-')) lineClass = 'text-red-500 bg-red-500/5 font-semibold';
                  return (
                ...
                <div className="flex flex-col items-center justify-center h-full text-zinc-500 text-xs">
                  <FileText size={32} className="mb-2 text-zinc-650" />
          ...
          <div className="w-96 border-l border-zinc-850 flex flex-col bg-zinc-950 shrink-0 overflow-y-auto">
            
            {/* Gate Analytics */}
            <div className="p-5 border-b border-zinc-850 space-y-4">
              <h3 className="text-xs font-bold text-zinc-400 uppercase tracking-wider">Gate Analytics</h3>
              
              {/* SVG circular progress chart */}
              <div className="flex justify-between items-center gap-4 bg-zinc-900/30 border border-zinc-850 p-3 rounded-lg">
                <div className="relative w-16 h-16 shrink-0 flex items-center justify-center">
                  ...
                  <span className="absolute text-[11px] font-bold text-white">{passPct}%</span>
                </div>
              ...
              <div className="space-y-2.5">
                ...
                <div className="space-y-2 bg-zinc-900/30 border border-zinc-850 p-3.5 rounded-lg">
                  {[
                    ...
                    <div key={row.label} className="space-y-1">
                      <div className="flex justify-between text-[10px] font-medium text-zinc-300 font-sans">
                        <span>{row.label}</span>
            ...
            <div className="p-5 border-b border-zinc-850 space-y-3">
              <h3 className="text-xs font-bold text-zinc-400 uppercase tracking-wider">Persona Audits Summary</h3>
                    ...
                    <div key={persona.id} className="flex gap-2.5 items-start">
                      ...
                      <div>
                        <div className="flex items-center gap-1.5">
                          <span className="text-[10px] font-bold text-zinc-300">{persona.label}</span>
                          <span className={`text-[8px] font-bold px-1 rounded uppercase tracking-wider ${
                            persona.status === 'passed' ? 'bg-green-500/10 text-green-400 border border-green-500/20' :
                            persona.status === 'failed' ? 'bg-red-500/10 text-red-400 border border-red-500/20' :
                            persona.status === 'running' ? 'bg-blue-500/10 text-blue-400 border border-blue-500/20 animate-pulse' :
            ...
            <div className="p-5 flex-1 flex flex-col justify-end gap-3 min-h-[200px]">
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  className="w-full h-16 p-2 text-xs bg-zinc-900 border border-zinc-800 rounded outline-none resize-none text-white focus:border-zinc-700"
                />
              </div>
              ...
              <div className="flex gap-2">
                <button
                  onClick={onClose}
                  className="flex-1 py-2 bg-zinc-900 hover:bg-zinc-850 border border-zinc-850 hover:border-zinc-800 text-zinc-300 font-semibold text-xs rounded transition"
                >
                  Cancel Audit
                </button>
                <button
